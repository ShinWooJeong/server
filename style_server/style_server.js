...
router.route('/textsearch/:query').get(function(req,res) {   ////////////////google 검색 api
  var query = req.params.query;
  var url = 'https://maps.googleapis.com/maps/api/place/textsearch/json?query=' + encodeURI(query) + '&key=AIzaSyCXnKkYfyqyacIivArtNQFgkypAj7HXJhw';
  console.log(url);
  
  requestUrl.get(url, function(error, response, body) {
    if(error){
      console.log('error in /textsearch/:query.... ', err);
...
router.route('/textsearch/nextpage/:nextpagetoken').get(function(req,res) {  ////////////////google 검색 api
  var nextpagetoken = req.params.nextpagetoken;
  var url = "https://maps.googleapis.com/maps/api/place/textsearch/json?pagetoken=" + encodeURI(nextpagetoken) + "&key=AIzaSyCXnKkYfyqyacIivArtNQFgkypAj7HXJhw";
  console.log(url);
